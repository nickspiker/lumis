...
pub fn debayer_with_pattern(buffer: &ImageBuffer, filter_pattern: FilterPattern) -> Result<Image> {
    ...
    for (top, left) in iproduct!(
        ((winy - 16)..(winy + height)).step_by(TS as usize - 32),
        ((winx - 16)..(winx + width)).step_by(TS as usize - 32)
    ) {
        ...
        //colour ratios in up/down/left/right directions
        // let mut cru: f32 = 0.0;
        // let mut crd: f32 = 0.0;
        // let mut crl: f32 = 0.0;
        // let mut crr: f32 = 0.0;
        ...
        //colour difference variances in vertical and horizontal directions
        ...
        //adaptive interpolation weight using variance of colour differences
        ...
        //colour ratios in diagonal directions
        ...
        (rrmin..rrmax).for_each(|rr| {
            let row = rr + top;
            (ccmin..ccmax).for_each(|cc| {
                let col = cc + left;
            ...
            (0..16).for_each(|rr| {
                (ccmin..ccmax).for_each(|cc| {
                    ...
                    rgb[(rrmax + rr) * TS + cc][c] =
                        (image[(height - rr - 2) * width + left + cc][c]) / pixel_value_max;
            ...
            (rrmin..rrmax).for_each(|rr| {
                (0..16).for_each(|cc| {
                    ...
                    rgb[rr * TS + ccmax + cc][c] =
                        (image[(top + rr) * width + (width - cc - 2)][c]) / pixel_value_max;
            ...
            (0..16).for_each(|rr| {
                (0..16).for_each(|cc| {
                    ...
                    rgb[(rrmax + rr) * TS + ccmax + cc][c] =
                        (image[(height - rr - 2) * width + (width - cc - 2)][c]) / pixel_value_max;
        ...
        if rrmin > 0 && ccmax < cc1 {
            (0..16).for_each(|rr| {
                (0..16).for_each(|cc| {
                    let c = FC!(rr, cc, filter_pattern);
                    rgb[(rr) * TS + ccmax + cc][c] =
                        (image[(32 - rr) * width + (width - cc - 2)][c]) / pixel_value_max;
                    cfa[(rr) * TS + ccmax + cc] = rgb[(rr) * TS + ccmax + cc][c];
                });
            });
        }
        if rrmax < rr1 && ccmin > 0 {
            (0..16).for_each(|rr| {
                (0..16).for_each(|cc| {
                    let c = FC!(rr, cc, filter_pattern);
                    rgb[(rrmax + rr) * TS + cc][c] =
                        (image[(height - rr - 2) * width + (32 - cc)][c]) / pixel_value_max;
                    cfa[(rrmax + rr) * TS + cc] = rgb[(rrmax + rr) * TS + cc][c];
                });
            });
        }
        ...
        (2..rr1 - 2).for_each(|rr| {
            ...
            while cc < cc1 - 2 {
                ...
                if FC!(rr, cc, filter_pattern) & 1 >= 1 {
                    ...
                    dgrbpsq1[indx] =
                        SQR!(cfa[indx] - cfa[indx - p1]) + SQR!(cfa[indx] - cfa[indx + p1]);
                    dgrbmsq1[indx] =
                        SQR!(cfa[indx] - cfa[indx - m1]) + SQR!(cfa[indx] - cfa[indx + m1]);
                }
                cc += 1;
                indx += 1;
            }
        ...
        (4..rr1 - 4).for_each(|rr| {
            ...
            while cc < cc1 - 4 {
                ...
                let cru: f32 = cfa[indx - v1] * (dirwts[indx - v2][0] + dirwts[indx][0])
                    / (dirwts[indx - v2][0] * (eps + cfa[indx])
                        + dirwts[indx][0] * (eps + cfa[indx - v2]));
                let crd: f32 = cfa[indx + v1] * (dirwts[indx + v2][0] + dirwts[indx][0])
                    / (dirwts[indx + v2][0] * (eps + cfa[indx])
                        + dirwts[indx][0] * (eps + cfa[indx + v2]));
                ...
                let guha: f32 = min!(clip_pt, cfa[indx - v1]) + 0.5 * (cfa[indx] - cfa[indx - v2]);
                let gdha: f32 = min!(clip_pt, cfa[indx + v1]) + 0.5 * (cfa[indx] - cfa[indx + v2]);
                let glha: f32 = min!(clip_pt, cfa[indx - 1]) + 0.5 * (cfa[indx] - cfa[indx - 2]);
                let grha: f32 = min!(clip_pt, cfa[indx + 1]) + 0.5 * (cfa[indx] - cfa[indx + 2]);
                ...
                let hwt: f32 = dirwts[indx - 1][1] / (dirwts[indx - 1][1] + dirwts[indx + 1][1]);
                let vwt: f32 = dirwts[indx - v1][0] / (dirwts[indx + v1][0] + dirwts[indx - v1][0]);
                ...
                if c & 1 >= 1 {
                    ...
                    if hcd[indx] > 0.0 {
                        if 3.0 * hcd[indx] > (ginth + cfa[indx]) {
                            ...
                            let hwt: f32 = 1.0 - 3.0 * hcd[indx] / (eps + ginth + cfa[indx]);
                    ...
                    if vcd[indx] > 0.0 {
                        if 3.0 * vcd[indx] > (gintv + cfa[indx]) {
                            ...
                            let vwt: f32 = 1.0 - 3.0 * vcd[indx] / (eps + gintv + cfa[indx]);
        ...
        (6..rr1 - 6).for_each(|rr| {
            ...
            while cc < cc1 - 6 {
                let mut dgrbvvaru: f32 = 4.0
                    * (vcdsq[indx] + vcdsq[indx - v1] + vcdsq[indx - v2] + vcdsq[indx - v3])
                    - SQR!(vcd[indx] + vcd[indx - v1] + vcd[indx - v2] + vcd[indx - v3]);
                let mut dgrbvvard: f32 = 4.0
                    * (vcdsq[indx] + vcdsq[indx + v1] + vcdsq[indx + v2] + vcdsq[indx + v3])
                    - SQR!(vcd[indx] + vcd[indx + v1] + vcd[indx + v2] + vcd[indx + v3]);
                let mut dgrbhvarl: f32 = 4.0
                    * (hcdsq[indx] + hcdsq[indx - 1] + hcdsq[indx - 2] + hcdsq[indx - 3])
        ...
        (8..rr1 - 8).for_each(|rr| {
            ...
            while cc < cc1 - 8 {
                let hvwtalt: f32 =
                    0.25 * (hvwt[indx - m1] + hvwt[indx + p1] + hvwt[indx - p1] + hvwt[indx + m1]);
                ...
                dgrb[indx][0] = hcd[indx] * (1.0 - hvwt[indx]) + vcd[indx] * hvwt[indx]; //evaluate colour differences
                rgb[indx][1] = cfa[indx] + dgrb[indx][0]; //evaluate G (finally!)

                if nyquist[indx] >= 1 {
                    dgrbh2[indx] =
                        SQR!(rgb[indx][1] - 0.5_f32 * (rgb[indx - 1][1] + rgb[indx + 1][1]));
                    dgrbv2[indx] =
                        SQR!(rgb[indx][1] - 0.5_f32 * (rgb[indx - v1][1] + rgb[indx + v1][1]));
                } else {
                    dgrbh2[indx] = 0.0;
                    dgrbv2[indx] = 0.0;
                }
        ...
        (8..rr1 - 8).for_each(|rr| {
            ...
            while cc < cc1 - 8 {
                ...
                let crse = 2.0 * (cfa[indx + m1]) / (eps + cfa[indx] + (cfa[indx + m2]));
                let crnw = 2.0 * (cfa[indx - m1]) / (eps + cfa[indx] + (cfa[indx - m2]));
                let crne = 2.0 * (cfa[indx + p1]) / (eps + cfa[indx] + (cfa[indx + p2]));
                let crsw = 2.0 * (cfa[indx - p1]) / (eps + cfa[indx] + (cfa[indx - p2]));
                ...
                if rbp[indx] < cfa[indx] {
                    rbp[indx] = if 2.0 * rbp[indx] < cfa[indx] {
                        ...
                        let pwt: f32 =
                            2.0 * (cfa[indx] - rbp[indx]) / (eps + rbp[indx] + cfa[indx]);
                        pwt * rbp[indx]
                            + (1.0 - pwt) * ULIM!(rbp[indx], cfa[indx - p1], cfa[indx + p1])
                    };
                }
                ...
                if rbm[indx] < cfa[indx] {
                    rbm[indx] = if 2.0 * rbm[indx] < cfa[indx] {
                        ...
                        let mwt: f32 =
                            2.0 * (cfa[indx] - rbm[indx]) / (eps + rbm[indx] + cfa[indx]);
                        mwt * rbm[indx]
                            + (1.0 - mwt) * ULIM!(rbm[indx], cfa[indx - m1], cfa[indx + m1])
                    }
                }
        ...
        (10..rr1 - 10).for_each(|rr| {
            let mut cc = 10 + (FC!(rr, 2, filter_pattern) & 1);
            let mut indx = rr * TS + cc;
            while cc < cc1 - 10 {
                let pmwtalt =
                    0.25 * (pmwt[indx - m1] + pmwt[indx + p1] + pmwt[indx - p1] + pmwt[indx + m1]);
                ...
                rbint[indx] =
                    0.5 * (cfa[indx] + rbm[indx] * (1.0 - pmwt[indx]) + rbp[indx] * pmwt[indx]); //this is R+B, interpolated
        ...
        (12..rr1 - 12).for_each(|rr| {
            ...
            while cc < cc1 - 12 {
                ...
                let cru: f32 = cfa[indx - v1] * 2.0 / (eps + rbint[indx] + rbint[indx - v2]);
                let crd: f32 = cfa[indx + v1] * 2.0 / (eps + rbint[indx] + rbint[indx + v2]);
                let crl: f32 = cfa[indx - 1] * 2.0 / (eps + rbint[indx] + rbint[indx - 2]);
                let crr: f32 = cfa[indx + 1] * 2.0 / (eps + rbint[indx] + rbint[indx + 2]);
                ...
                //interpolated G via adaptive weights of cardinal evaluations
                let mut gintv: f32 = (dirwts[indx - v1][0] * gd + dirwts[indx + v1][0] * gu)
                    / (dirwts[indx + v1][0] + dirwts[indx - v1][0]);
                let mut ginth: f32 = (dirwts[indx - 1][1] * gr + dirwts[indx + 1][1] * gl)
                ...
                if gintv < rbint[indx] {
                    gintv = if 2.0 * gintv < rbint[indx] {
                        ...
                        let vwt: f32 = 2.0 * (rbint[indx] - gintv) / (eps + gintv + rbint[indx]);
                        vwt * gintv + (1.0 - vwt) * ULIM!(gintv, cfa[indx - v1], cfa[indx + v1])
                    };
                }
                if ginth < rbint[indx] {
                    ginth = if 2.0 * ginth < rbint[indx] {
                        ...
                        let hwt: f32 = 2.0 * (rbint[indx] - ginth) / (eps + ginth + rbint[indx]);
                        hwt * ginth + (1.0 - hwt) * ULIM!(ginth, cfa[indx - 1], cfa[indx + 1])
                    }
                }
        ...
        (13 - ey..rr1 - 12).step_by(2).for_each(|rr| {
            let mut cc = 13 - ex;
            let mut indx = rr * TS + cc;
            while cc < cc1 - 12 {
        ...
        (16..rr1 - 16).for_each(|rr| {
            ...
            while cc < cc1 - 16 {
                ...
                let indx = row * width + col;
                for c in 0..3 {
                    image[indx][c] = CLIP!(pixel_value_max * rgb[rr * TS + cc][c] + 0.5_f32);
                }
                cc += 1
            }
        });
